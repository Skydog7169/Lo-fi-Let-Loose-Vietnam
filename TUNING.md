...
- **AI-vs-AI after the offense fixes (assault, spawn lock, firing reveals, attacker manpower ×1.35): ≈6 of 8 attacker wins**, some in ~4 min. If the human attacker now steamrolls, first knobs: `ATTACKER_MANPOWER_MULT` → 1.2, `ACTIVE_POINT_SPAWN_LOCK_R` → 150, `CAPTURE_SECONDS_PER_DOT` → 55.
